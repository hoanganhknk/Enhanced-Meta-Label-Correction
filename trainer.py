...
class Trainer:
    def __init__(self, rank, args, main_net, meta_net, enhancer,
        ...
        # Lazily create writer to avoid importing TF/XLA stuff unless needed
        self.writer = None
        if rank == 0 and (not getattr(args, "no_tb", False)):
            try:
                from tensorboardX import SummaryWriter  # usually cleaner than torch.utils.tensorboard
            except Exception:
    ...
    def _setup_training(self):
        """Fetch optimizers and schedulers for training the student, teacher and enhancer."""
        args = self.args
        ...
        self.meta_opt = torch.optim.SGD(meta_params, lr=args.meta_lr, weight_decay=args.wdecay)
        self.enhancer_opt = torch.optim.SGD(enhancer_params, lr=args.meta_lr, weight_decay=args.wdecay)

        # Parse milestones string like "20" or "20,30"
    ...
    def _training_iter(self, data_s, target_s, data_g, target_g):
        ...
        kwargs = {
            ...
            "main_opt": self.main_opt,
            "teacher": self.meta_net,
            "teacher_opt": self.meta_opt,
            "enhancer": self.enhancer,
            "enhancer_opt": self.enhancer_opt,
            "data_s": data_s,
            "target_s": target_s,
            "data_g": data_g,
            "target_g": target_g,
        ...
        # Logging only (avoid dist.reduce / TB when not needed)
        if self.writer is not None and (self.args.steps % self.args.every == 0):
                ...
                *data_g, target_g = next(self.gold_loader)
                ...
                if self.args.steps % self.args.gradient_steps == 0:
                    if self.args.gradient_steps == 1:
                        ...
                        data_s_cat = torch.cat(all_data_s)
                        target_s_cat = torch.cat(all_target_s)
                        self._training_iter(data_s_cat, target_s_cat, data_g, target_g)
                        all_data_s, all_target_s = [], []
    ...
    def _epoch_evaluation(self):
        """Evaluate epoch and save best model if necessary."""
